    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            fixed4 frag (v2f i, fixed facing : VFACE) : SV_Target
            {
                ...
                col += tex2D(_MainTex, i.uv + offset.zy); 
                col += tex2D(_MainTex, i.uv + offset.xy); 
                col += tex2D(_MainTex, i.uv + offset.yz);
                col += tex2D(_MainTex, i.uv);
                col += tex2D(_MainTex, i.uv + offset.rb);
                col += tex2D(_MainTex, i.uv + offset.yx);
                col += tex2D(_MainTex, i.uv + offset.zx);
                col += tex2D(_MainTex, i.uv + offset.xx);

                return facing > 0 ? col / 9 : tex2D(_MainTex, i.uv);
            }
            ENDCG
        }
    }
